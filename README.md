...
- **完成与受阻**：全部口令合法执行完毕即显示走台完成（并提示是否回到空载归位）；遇到非法卡则**停在执行前状态**，给出受阻卡号与原因。重复点击不会越过末张或受阻卡。
- **编辑锁定**：走台进行中锁定牌库、排序、重量、删除与草稿操作，轨迹与实时结论也不再展示；完成或受阻后恢复编辑与实时裁决视图。
- 会话状态（待命 / 进行中 / 完成 / 受阻）由 `src/domain/session.ts` 的纯函数管理，仅存于内存：刷新页面仍按现有空序列启动，回到待命。
