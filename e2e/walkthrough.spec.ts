...
test('标准闭环逐张完成：操作者只看到当前口令、吊杆状态与剩余张数', async ({ page }) => {
  ...
  // 走台进行中：整条轨迹、实时结论与状态面板不再展示，序列卡也不带裁决角标
  ...
  await expect(page.getByTestId('card-status')).toHaveCount(0);
...
test('走台进行中锁定编辑入口，完成后重新可用', async ({ page }) => {
  ...
  await expect(page.getByTestId('card-weight').first()).toBeDisabled();
  await expect(page.getByTestId('card-delete').first()).toBeDisabled();
  await expect(page.getByTestId('card-up').nth(1)).toBeDisabled();
  await expect(page.getByTestId('card-down').first()).toBeDisabled();
  await expect(page.getByTestId('save-draft')).toBeDisabled();
  await expect(page.getByTestId('restore-draft')).toBeDisabled();
  await expect(page.getByTestId('sequence-card').first()).toHaveAttribute('draggable', 'false');

  // 执行完全部 6 张 → 走台完成
  await executeAll(page, 6);
  await expect(page.getByTestId('walkthrough-result')).toContainText('走台完成');

  // 结束后编辑入口恢复可用，实时裁决视图同步恢复
  ...
  await expect(page.getByTestId('card-up').nth(1)).toBeEnabled();
  await expect(page.getByTestId('card-down').first()).toBeEnabled();
